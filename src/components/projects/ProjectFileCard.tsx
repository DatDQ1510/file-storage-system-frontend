...
interface IProjectFileCardProps {
  fileId?: string;
  name: string;
  size: string;
  lastModified: string;
...
export const ProjectFileCard = ({
  ...
  const handleRenameConfirm = async (newName: string) => {
    if (!menuActions?.onRename || !fileId) return;
  ...
  const handleDeleteConfirm = async () => {
    if (!menuActions?.onDelete || !fileId) return;
    setIsActionLoading(true);
    try {
  ...
  return (
    <>
      <div
        className={cn(
          "group relative flex min-h-[120px] flex-col justify-between rounded-lg border-2 bg-white px-4 py-3 shadow-sm transition-all duration-200 dark:bg-slate-900",
          isActive
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          {hasMenu && (
            <div ref={menuRef} className="relative">
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full z-20 mt-1 min-w-[140px] rounded-lg border border-slate-200 bg-white py-1 shadow-lg dark:border-slate-700 dark:bg-slate-900">
